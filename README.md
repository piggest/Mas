...
- **動画撮影**: 画面領域をMP4（H.264）で録画。トリミング・GIFエクスポート対応
- **シャッター機能**: 時限・インターバル・変化検知・プログラマブルの4モード
- **豊富な編集ツール**: ペン、マーカー、矢印、四角、丸、集中線、テキスト、モザイク、トリミング
...
1. ZIPをダウンロードして展開
2. `Mas.app`を`Applications`フォルダに移動
3. 画面収録の権限を許可
...
2. ドラッグで録画範囲を選択すると録画が開始されます（MP4/H.264、20FPS）
3. 停止ボタンを押すと動画ファイルが生成されます
4. ビデオプレイヤーで再生・5秒スキップ・フレーム移動・速度変更が可能
5. トリムモードで範囲を切り出し、GIFエクスポートにも対応
...
- **キャプチャアクション**: 右上のボタンで再キャプチャ/GIF録画/動画録画（右クリックでモード切替）
...
メニューバーアイコンから「設定」を選択、または `⌘,` で設定画面を開けます。設定は5つのタブに分かれています（開発者モード有効時は6つ）。
...
プロプライエタリ。Copyright (c) 2026 piggest. All rights reserved.
配布バイナリは個人・商用問わず無償で利用できますが、再配布・改変は許可されていません。詳細は [LICENSE](LICENSE) を参照。
