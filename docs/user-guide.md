...
キャプチャからアノテーション、GIF録画、動画撮影、テキスト認識まで、スクリーンショットに必要な機能をコンパクトにまとめています。
...
1. [Releases](https://github.com/piggest/Mas/releases) から最新の `Mas-x.x.x.zip` をダウンロード
2. ZIPを展開して `Mas.app` を `/Applications` フォルダに移動
3. 画面収録の権限を許可してください
...
録画完了後、動画はエディターウィンドウで開かれます。ウィンドウ下部にフローティングツールバーが表示され、再生操作が可能です。

#### 操作

| 操作 | 説明 |
|------|------|
| 再生/一時停止 | 動画のプレビュー再生・停止 |
| 5秒戻し/5秒進み | 前後にジャンプ |
| 前フレーム/次フレーム | 1フレーム単位で移動 |
| フレームスライダー | 任意の位置にシーク |
| 再生速度 | 0.5x / 1x / 2x から選択 |
...
ビデオプレイヤーのツールバーからトリムモードに入れます。

1. ツールバーの「トリム」ボタンをクリック
2. スライダーでトリム開始位置と終了位置を設定
3. プレビューで確認
4. 「確定」で指定範囲を切り出した新しい動画ファイルが生成されます

### 動画からGIFへのエクスポート

動画の全体またはトリム範囲をアニメーションGIFに変換できます。

1. ビデオプレイヤーのツールバーで「GIF」ボタンをクリック
2. トリム中の場合はトリム範囲が、そうでなければ動画全体がGIFに変換されます
3. エクスポートの進捗がプログレスバーで表示されます
4. 完了するとGIFがエディターウィンドウで開かれます

> 録画コントロールウィンドウは動画に写り込みません。
...
キャプチャアクションボタンは撮影モードに応じてアイコンが自動切替されます（スクショ→📷、GIF→⏺、動画→🎥）。右クリックで他のモードに変更できます。
...
メニューの「設定...」（`⌘,`）から設定ウィンドウを開けます。5つのタブで構成されています。
